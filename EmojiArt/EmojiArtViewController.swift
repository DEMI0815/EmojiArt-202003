...
class EmojiArtViewController: UIViewController, UIDropInteractionDelegate, UIScrollViewDelegate, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UICollectionViewDragDelegate, UICollectionViewDropDelegate {
                ...
                imageFetcher = ImageFetcher(fetch: url) { (url, image) in
                    DispatchQueue.main.async {
                        ...
                        newValue?.emojis.forEach {
                            ...
                            self.emojiArtView.addLabel(with: attributedText, at: CGPoint(x: $0.x, y: $0.y))
                        }
                    }
                }
            }
    ...
    func collectionView(_ collectionView: UICollectionView, dropSessionDidUpdate session: UIDropSession, withDestinationIndexPath destinationIndexPath: IndexPath?) -> UICollectionViewDropProposal {
        if let indexPath = destinationIndexPath, indexPath.section == 1 {
            let isMySelf = (session.localDragSession?.localContext as? UICollectionView) == collectionView
            return UICollectionViewDropProposal(operation: isMySelf ? .move : .copy, intent: .insertAtDestinationIndexPath)
        } else {
                ...
                item.dragItem.itemProvider.loadObject(ofClass: NSAttributedString.self) { (provider, error) in
                    DispatchQueue.main.async {
                        if let attributedString = provider as? NSAttributedString {
                            placeholderContext.commitInsertion { (insertIndexPath) in
                                self.emojis.insert(attributedString.string, at: insertIndexPath.item)
                            }
                        } else {
